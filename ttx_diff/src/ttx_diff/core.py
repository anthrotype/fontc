...
def get_crate_path(cli_arg: Optional[str], root_dir: Optional[Path], crate_name: str) -> Path:
    """Get path to a crate binary, building it if in fontc repo, or finding in PATH.

    Args:
        cli_arg: Path provided via CLI flag
    ...
    """
    if cli_arg:
        bin_path = Path(cli_arg)
        if not bin_path.is_file():
            sys.exit(f"Specified {crate_name} path '{bin_path}' does not exist")
        return bin_path
...
def main(argv):
    ...
    if shutil.which("fontmake") is None:
        sys.exit("fontmake not found. Please install it: pip install fontmake")
    if shutil.which("ttx") is None:
        sys.exit("ttx not found. Please install it: pip install fonttools")
